...
public class ProductController {
	...
	public ModelAndView seeProduct(Model model, @RequestParam("id") String p_id, ModelAndView mav, HttpSession session)
			throws Exception {
		log.info("p_id " + p_id);
...
//	    likeVO.setUserid((String) session.getAttribute("userID"));
	...
	public void GetList(Model model, @RequestParam("id") String p_id, HttpSession session) throws Exception {
		String userid = (String) session.getAttribute("userID");
		//String p_id = (String) request.getAttribute("id");

		// p id로 product 정보 가져오기
		ProductVO product = service.product(p_id);
		
		//유저 아이디로 유저 정보 가져오기
		MemberVO member = memberService.userInfoView("admin");
