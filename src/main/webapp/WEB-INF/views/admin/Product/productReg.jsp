...
<script>
	...
	function register() {
		...
		if($("#id_chk").val() == "false"){
			...
			if(idInput.value != ''){
				window.open("pidCheck?p_id="+idInput.value,"","width=400, height=200");
			}  
			return false;
		}
		...
		<form id="WriteForm" class="WriteForm" method="POST">
			...
			<div class="row">
				<label class="title"><p class="mustIn">*</p> 상품가격</label> <input type="text"
					class="p_price" id="p_unitPrice" name="p_unitPrice">
			</div>

			<div class="row">
				<label class="title">상세 정보</label>
				<textarea id="p_description" name="p_description" cols="50" rows="2"
					class="p_description"></textarea>
			</div>

			<div class="row">
				<label class="title">제조사</label> <input type="text"
					class="p_manufacturer" id="p_manufacturer" name="p_manufacturer">
			</div>

			<div class="row">
				<label class="title">분류</label> 
			<select class="p_category" id="p_category" name="p_category">
			...
			<div class="row">
				...
				<div class="col-sm5">
					<input type="radio" name="p_condition" value="New"> 신규 제품 <input
						type="radio" name="p_condition" value="Old"> 중고 제품 <input
						type="radio" name="p_condition" value="Refurbished"> 재생 제품
				</div>
			</div>
